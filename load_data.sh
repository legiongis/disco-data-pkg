...
python manage.py packages -o import_reference_data -s $HERPATH/reference_data/DISCO_concepts.xml -ow overwrite -st keep
...
python manage.py packages -o import_reference_data -s $HERPATH/reference_data/DISCO_collections.xml -ow overwrite -st keep
...
python manage.py packages -o add_mapbox_layer -j $HERPATH/mapbox_styles/3d_buildings/style.json -n "3d Buildings"
